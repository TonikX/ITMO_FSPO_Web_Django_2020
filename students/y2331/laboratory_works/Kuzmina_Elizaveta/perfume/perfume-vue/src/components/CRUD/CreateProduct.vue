<template>
  <div>
    <mu-dialog title="Создание товара" width="400" max-width="80%" :esc-press-close="false"
      ...
      <mu-form :model="form" :label-position="labelPosition" ref="form">
        ...
        <mu-form-item prop="type" label="Тип" :rules="lineRule">
        ...
        <mu-form-item prop="sex" label="Пол" :rules="lineRule">
    ...
    methods: {
      ...
      closeSaveAlertDialog() {
        this.$refs.form.validate().then((result) => {
          if (result === false || this.nameFabric === '') {
            alert('Какие-то поля заполнены неверно или не заполнены.');
          } else {
            $.ajax({
              url: "http://127.0.0.1:8000/api/v1/perfume/product/",
              type: "POST",
              data: {
                name: this.form.name,
