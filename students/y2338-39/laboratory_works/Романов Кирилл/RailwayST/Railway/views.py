...
def showCart(request):
    return render(request, 'Railway/Cart.html', context={'tck': Ticket.objects.all()})

def authen(request):
    return render(request, 'AuthForm.html')

def authNone(request):
    return redirect(reverse('station_url'))
...
class TicketPurchase(LoginRequiredMixin, View):
    raise_exception = True

    def get(self, request):
        cost = TrainCost.objects.all()
        cont = {
            'Cost':  cost,
            'Ticket': TicketPurchaseForm(),

        }
        return render(request, 'Railway/CartPurchase.html', context=cont)

    def post(self, request):
        form = TicketPurchaseForm(request.POST)
        if (form.is_valid()):
            form.save()
            return redirect(reverse('ticketPurchase'))
        return render(request, 'Railway/CartPurchase.html', context={'Cost':  TrainCostForm(),'Ticket': TicketPurchaseForm(),})
