<template>
    ...
    <vue-headful title="Авторизация"/>
        <div class="form-pannel" style="width: 40%; margin-top:100px; margin-left:350px">
            <mu-row gutter>
        ...
        methods: {
            ...
            conf_login() {
                if (this.login && this.password) {
                    $.ajax({
                        ...
                        error: (response) => {
                            if(response.status == 400) {
                                this.alert_text = "Исправте неправельно введённые поля";
                                this.open_alert_dialog()
                            }
                            else {
                                this.alert_text = "Ошибка сервера";
                                this.open_alert_dialog()
                            }
                        }
                    })
                    ...
                    this.alert_text = "Заполните все формы";
                    this.open_alert_dialog()
                }
            },
            open_alert_dialog() {
