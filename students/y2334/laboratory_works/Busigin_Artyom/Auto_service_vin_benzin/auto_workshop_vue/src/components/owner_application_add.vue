<template>
    ...
    <vue-headful title="Создание заявки"/>
        <div class="form-pannel" style="width: 50%; margin-top: 100px; margin-left: 300px">
            <mu-row gutter>
                <mu-col span=3 class="form-pannel-cell">
                    <label class="form-label" style="margin-left: 6px"><strong>Автомобиль:</strong></label>
                </mu-col>
            ...
            <mu-row gutter>
                <mu-col span=3 class="form-pannel-cell">
                    <label class="form-label" style="margin-left: 4px"><strong>Дата записи:</strong></label>
                </mu-col>
                <mu-col span=9 class="form-pannel-cell">
                    <input class="form-input" v-model="date" type="date" style="margin-left: -267px"/>
                </mu-col>
            </mu-row>
            <mu-row>
                <label style="float: left; margin-left: 21px"><strong>Описание:</strong></label>
            </mu-row>
            <mu-row>
                <textarea rows="10" class="form-textarea" style="margin-left: 20px" v-model="description"></textarea>
            </mu-row>
            ...
            conf_application_add() {
            	if (this.auto_id && this.workshop_id && this.date && this.description) {
                    $.ajax({
                        ...
                        success: (response) => {
                            this.$router.push({name: "owner_applications_rout", query: {"outer_alert": "Заявка сформирована"}})
                        },
                        error: (response) => {
                            if(response.status == 400) {
                    			this.alert_text = "Неверное поле";
                    			this.open_alert_dialog()
                            }
                            else {
                    			this.alert_text = "Ошибка на стороне сервера";
                    ...
                    this.alert_text = "Заполните поля";
                    this.open_alert_dialog()
                }
            },
            open_alert_dialog() {
