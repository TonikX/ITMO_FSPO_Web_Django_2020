<template>
    ...
    <vue-headful title="Добавление автомобиля"/>
        <div class="form-pannel" style="width: 41%; margin-top: 100px; margin-left: 300px">
            <mu-row gutter>
            ...
            conf_auto_add() {
                if (this.number && this.brand && this.model && this.year && this.tech_passport) {
                    $.ajax({
                        ...
                        success: (response) => {
                            this.$router.push({name: "owner_autos_rout", query: {"outer_alert": "Автомобиль добавлен"}})
                        },
                        error: (response) => {
                            if(response.status == 400) {
                                this.alert_text = "Заполните поля праильно";
                    ...
                    this.alert_text = "Правильно заполните поля";
                    this.open_alert_dialog()
                }
            },
            open_alert_dialog() {
